...
setwd("C:/Users/amurgueitio/Documents/Multistage_omics/Orthogroups4sp")#OfficePC

setwd("C:/Users/amurg/OneDrive/Documentos/GitHub/Multistage_omics/Orthogroups/4 species")#MyPC
...
source("C:/Users/amurgueitio/Documents/GitHub/Multistage_omics/Orthogroups/Scripts/modified_venn_diagram.R")#Office
source("modified_venn_diagram.R")#Home

# Read in data
x <- fread("Orthogroups.tsv")
...
# Define the custom color-blind-friendly palette (order must match species below)
library(VennDiagram)
...
cb_palette <- c(
  Montipora = "#4dac26",   # dark green
  Pocillopora = "#f1b6da",  # light purple
  Stylophora = "#d01c8b",  # dark purple
  Acropora = "#b8e186"    # light green
)
...
venn.diagram(
  ...
  filename = "venn_diagram_coral_species_cb_trial.png",
  output = TRUE,
  imagetype = "png",
  scaled = FALSE,
  col = "black",
  ...
  cat.col = cb_palette,
  cat.cex = 0.95,
  cat.dist = c(0.30, 0.30, 0.35, 0.35),  # Slightly increase distance for Acropora tenuis and Stylophora pistillata
  cat.pos = c(-15, 10, -15, 10),          # Lower Acropora tenuis and Stylophora pistillata
  margin = 0.30,
  euler.d = FALSE
)
